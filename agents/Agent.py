...
from config_files.config import get_config
...
class Agent:
    def __init__(self, config):
        ...
        classifier = 'swinchex'
        #classifier = 'chexnet' # for 'swinchex' comment this line
        
        if classifier == 'chexnet':
            self.ac_model = torch.load('./networks/pretrained_classifier.pth', map_location='cpu')['model']
            ...
            config = get_config("./config_files/swin_large_patch4_window7_224.yaml")
            self.ac_model = SwinTransformer(img_size=config.DATA.IMG_SIZE,
            ...
            self.ac_model = self.ac_model.to(device)  # Move the model to the defined device
            checkpoint = torch.load('networks/ckpt_epoch_60.pth', map_location=device)  # Ensure the checkpoint is loaded to the right device
            sd = self.ac_model.state_dict()
            for key, value in checkpoint['model'].items():
                if key != 'head.weight' and key != 'head.bias':
                    sd[key] = value
            self.ac_model.load_state_dict(sd)
            self.ac_loss = Loss(ac_model=self.ac_model).cuda()
        ...
        #self.criterion_ac = nn.BCELoss().cuda()    # This is for ChexNet
        self.criterion_ac = nn.CrossEntropyLoss().cuda()
