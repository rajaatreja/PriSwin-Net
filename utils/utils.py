...
from utils.datasets.Dataset import Dataset
from utils.datasets.SiameseDataset import SiameseDataset
from utils.datasets.DatasetPretrain import DatasetPretrain
...
def train(generator, training_loader, gauss_filter, grid_identity, mu, ac_loss, verification_loss, ac_loss_weight, 
    ...
    for i, batch in enumerate(training_loader):
        ...
        # Print information
        print('Epoch [%d/%d], Iteration [%d/%d], Verification Loss (ver_loss): %.4f' % (epoch + 1, n_epochs, i + 1, len(training_loader), ver_loss.item()))
...
def validate(generator, validation_loader, gauss_filter, grid_identity, mu, ac_loss, verification_loss, ac_loss_weight, 
    ...
    with torch.no_grad():
        for i, batch in enumerate(validation_loader):
            ...
            # Print information
            print('Epoch [%d/%d], Iteration [%d/%d], Verification Loss (ver_loss): %.4f' % (epoch + 1, n_epochs, i + 1, len(validation_loader), ver_loss.item()))
...
def train_snn(perturbation_type, net, perturbation_net, grid_identity, gauss_filter, mu, training_loader, criterion,
    ...
    for i, batch in enumerate(training_loader):
        ...
        #print('Epoch [%d/%d], Iteration [%d/%d], Loss: %.4f' % (epoch + 1, n_epochs, i + 1, len(training_loader),loss.item()))
...
def validate_snn(perturbation_type, net, perturbation_net, grid_identity, gauss_filter, mu, validation_loader,
    ...
    with torch.no_grad():
        for i, batch in enumerate(validation_loader):
            ...
            #print('Epoch [%d/%d], Iteration [%d/%d], Loss: %.4f' % (epoch + 1, n_epochs, i + 1, len(validation_loader),loss.item()))
