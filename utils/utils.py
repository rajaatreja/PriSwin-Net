...
def train(generator, training_loader, gauss_filter, grid_identity, mu, ac_loss, verification_loss, ac_loss_weight, 
    ...
    for i, batch in enumerate(training_loader):
        ...
        for j in target_labels:
            target_tensor.append(j)
        
        loss_ac = criterion_ac(outputs_ac[0], target_tensor[0].long())
        for k in range(1, len(target_tensor)):
            loss_ac += criterion_ac(outputs_ac[k], target_tensor[k].long())
